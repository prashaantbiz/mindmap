...
// Resilient DB layer providing both PostgreSQL Prisma execution and local fallback
  ...
  try {
    if (fs.existsSync(dataFilePath)) {
      ...
      parsed.projects = (parsed.projects || []).map((p: any) => ({
        id: p.id,
        title: p.title || p.name || "Untitled Mind Map",
        description: p.description || null,
        thumbnailUrl: p.thumbnailUrl || null,
        nodeCount: p.nodeCount || 5,
...
export const db = {
  user: {
    async findUnique({ where }: { where: { email?: string; id?: string } }) {
      try {
        if (prisma) {
          return await prisma.user.findUnique({ where: where as any, include: { projects: true } });
        }
      } catch {}
      ...
      const projects = store.projects.filter((p) => p.userId === found.id);
    ...
    async delete({ where }: { where: { id?: string; email?: string } }) {
      try {
        if (prisma) {
          return await prisma.user.delete({ where: where as any });
        }
      } catch {}
      const store = readDevStore();
      ...
      if (user) {
        ...
        const userProjects = store.projects.filter((p) => p.userId === user.id);
        const userProjectIds = new Set(userProjects.map((p) => p.id));
        store.projects = store.projects.filter((p) => p.userId !== user.id);
        store.mindMapNodes = store.mindMapNodes.filter((n) => !userProjectIds.has(n.projectId));
        store.mindMapEdges = store.mindMapEdges.filter((e) => !userProjectIds.has(e.projectId));
        store.projectShares = store.projectShares.filter((s) => !userProjectIds.has(s.projectId));
        writeDevStore(store);
      }
      return user || null;
    },
  ...
  project: {
    async findMany({
      ...
      try {
        if (prisma) {
          return await prisma.project.findMany({ where: where as any, orderBy: orderBy as any });
        }
      } catch {}
      const store = readDevStore();
      let projects = store.projects.filter((p) => p.userId === where.userId);

      if (where.isArchived !== undefined) {
        projects = projects.filter((p) => p.isArchived === where.isArchived);
      }

      if (where.folder) {
        projects = projects.filter((p) => p.folder?.toLowerCase() === where.folder?.toLowerCase());
      }
    ...
    async findUnique({ where }: { where: { id: string } }) {
      try {
        if (prisma) {
          return await prisma.project.findUnique({ where: where as any, include: { nodes: true, edges: true } });
        }
      } catch {}
      const store = readDevStore();
    ...
    async update({ where, data }: { where: { id: string }; data: any }) {
      try {
        if (prisma) {
          return await prisma.project.update({ where: where as any, data: { ...data, updatedAt: new Date() } });
        }
      } catch {}
      const store = readDevStore();
      const index = store.projects.findIndex((p) => p.id === where.id);
    ...
    async delete({ where }: { where: { id: string } }) {
      try {
        if (prisma) {
          return await prisma.project.delete({ where: where as any });
        }
      } catch {}
      const store = readDevStore();
      const found = store.projects.find((p) => p.id === where.id);
      store.projects = store.projects.filter((p) => p.id !== where.id);
      store.mindMapNodes = store.mindMapNodes.filter((n) => n.projectId !== where.id);
      store.mindMapEdges = store.mindMapEdges.filter((e) => e.projectId !== where.id);
      writeDevStore(store);
      return found;
    },
  },
  ...
  canvas: {
    async getProjectCanvas(projectId: string, projectTitle: string = "Central Idea", template: string = "blank") {
      ...
      try {
        if (prisma) {
          ...
          if (nodes.length === 0) {
            // Provision default root node + starter branches
      ...
      // Fallback dev store
      const store = readDevStore();
      let nodes = store.mindMapNodes.filter((n) => n.projectId === projectId);
      let edges = store.mindMapEdges.filter((e) => e.projectId === projectId);
      ...
      if (nodes.length === 0) {
        ...
        writeDevStore(store);
        nodes = store.mindMapNodes.filter((n) => n.projectId === projectId);
        edges = store.mindMapEdges.filter((e) => e.projectId === projectId);
      }
    ...
    async saveProjectCanvas(
      ...
      // Fallback dev store
      const store = readDevStore();
      store.mindMapNodes = store.mindMapNodes.filter((n) => n.projectId !== projectId);
      store.mindMapEdges = store.mindMapEdges.filter((e) => e.projectId !== projectId);
  ...
  verificationToken: {
    async create({
      ...
      try {
        if (prisma) {
          return await prisma.verificationToken.create({ data: data as any });
        }
      } catch {}
      const store = readDevStore();
    ...
    async findUnique({ where }: { where: { token: string } }) {
      try {
        if (prisma) {
          return await prisma.verificationToken.findUnique({ where: where as any });
        }
      } catch {}
      const store = readDevStore();
      return store.verificationTokens.find((t) => t.token === where.token) || null;
    },

    async delete({ where }: { where: { token: string } }) {
      try {
        if (prisma) {
          return await prisma.verificationToken.delete({ where: where as any });
        }
      } catch {}
      const store = readDevStore();
      const found = store.verificationTokens.find((t) => t.token === where.token);
      store.verificationTokens = store.verificationTokens.filter(
  ...
  projectShare: {
    async findFirst({ where }: { where: { projectId?: string; token?: string } }) {
      try {
        if (prisma) {
          return await (prisma as any).projectShare.findFirst({ where });
        }
      } catch {}
      const store = readDevStore();
      return (
    ...
    async upsert({
      ...
      try {
        if (prisma) {
          const existing = await (prisma as any).projectShare.findFirst({
            where: { projectId },
          });
          if (existing) {
            return await (prisma as any).projectShare.update({
              where: { id: existing.id },
              data: { accessLevel, updatedAt: new Date() },
            });
          } else {
            return await (prisma as any).projectShare.create({
              data: {
                projectId,
                accessLevel,
                token: shareToken,
      ...
      const store = readDevStore();
      const index = store.projectShares.findIndex((s) => s.projectId === projectId);
      if (index !== -1) {
    ...
    async delete({ where }: { where: { projectId?: string; token?: string } }) {
      try {
        if (prisma) {
          const existing = await (prisma as any).projectShare.findFirst({ where });
          if (existing) {
            return await (prisma as any).projectShare.delete({ where: { id: existing.id } });
          }
        }
      } catch {}
