...
interface FloatingNodeToolbarProps {
  ...
  onAddChild: (direction?: "right" | "left") => void;
...
export function FloatingNodeToolbar({
  ...
  return (
    ...
    >
      ...
      {/* 1. AI Sparkle Brainstorm */}
      <button
        type="button"
        onClick={handleAIBrainstorm}
        className="h-7 w-7 rounded-xl flex items-center justify-center text-purple-500 hover:bg-purple-500/15 transition-colors"
        title="AI Brainstorm: Generate sub-topics"
      >
        <Sparkles className="h-4 w-4" />
      </button>

      <span className="h-3.5 w-px bg-border/80 mx-0.5" />

      {/* 2. Color & Style Palette Dropdown */}
      <DropdownMenu>
      ...
      {/* 6. More Actions Dropdown (Duplicate, Collapse, Delete) */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="h-7 w-7 rounded-xl flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
            title="More node options"
          >
